    ...
    <div class="row">
        <div class="col-md-12">
            <section class="panel">
                ...
                <?php
                    $URS_ID = $_GET['receiveursid'];
                    $idofysir = $_SESSION['myoid']; 
                    $empid = $_SESSION['myid']; 

                    $sql = "SELECT * FROM `ams_t_user_request_summary` AS URS INNER JOIN `ams_t_user_request` AS UR ON UR.URS_ID = URS.URS_ID INNER JOIN `ams_r_employee_profile` AS EP ON UR.EP_ID = EP.EP_ID INNER JOIN `ams_r_office` AS O ON EP.O_ID = O.O_ID where URS.URS_ID = $URS_ID";

                    $result= mysqli_query($connection, $sql) or die("Bad Query: $sql");

                    while($row = mysqli_fetch_assoc($result))  
                    {                              
                        $rdate = $row['URS_REQUEST_DATE'];
                        $urspos = $row['URS_PURPOSE']; 
                        $reqno = $row['URS_NO'];
                        $statsss = $row['URS_STATUS_TO_PO'];       
                    }                       
                ?>
                ...
                <div class="panel-body">
                    <div class="row group">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Status</label>
                                <input type="text" value="<?php echo $statsss; ?>" class="form-control" style="color: black;" disabled />
                            </div>
                        </div> 
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea style="color: black; word-wrap: break-word; resize: none; height: 85px;" class="form-control" maxlength="200" id="" required="" disabled><?php echo $urspos; ?></textarea>
                    ...
                    <div class="adv-table">
                        <table class="display table table-bordered table-striped classtbl">
                            ...
                            <tbody>
                                <?php 
                                    $URS_NO = $_GET['receiveursid'];
                                    $EID = $_SESSION['myid'];

                                    $sql_query = "SELECT AL.AL_NAME ANAME,CONCAT(EP.EP_FNAME,' ', EP.EP_LNAME) EPNAME, UR.`UR_UNIT` unit ,UR.`UR_QUANTITY` qty from ams_t_user_request UR INNER JOIN ams_r_asset_library AL on UR.AL_ID = AL.AL_ID INNER JOIN ams_r_employee_profile EP on UR.EP_ID = EP.EP_ID WHERE  UR.`URS_ID`= $URS_NO";
                                ...
                                ?>
                                    ...
                                    <tr>
                                        <td id="alname"> <?php echo $row['ANAME'];  ?></td>
                                        <td id="urunit"> <?php echo $row['unit'];  ?></td>
                                        <td id="urqty"> <?php echo $row['qty'];  ?></td>  
                                    </tr>
                    ...
                    <div class="row group">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <a class="btn btn-default" href="DUListOfRequest.php">Back</a>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="display table table-bordered table-striped" id="dynamic-table">
                                    <thead>
                                      ...
                                      <tbody>
                                        ...
                                        $URS_NO = $_GET['receiveursid'];
                                        $EID = $_SESSION['myid'];
                                        $sql_query = "SELECT AL.AL_NAME name,`UR_UNIT` unit ,`UR_QUANTITY` qty from ams_t_user_request UR 
                                        INNER JOIN ams_r_asset_library AL on UR.AL_ID = AL.AL_ID
                                        WHERE UR.`EP_ID` =$EID AND UR.`URS_ID`= $URS_NO";
                                        $query = mysqli_query($connection,$sql_query);
                                        while($row = mysqli_fetch_array($query)){
                                            ?>
                                        <tr>
                                            <td id="alname"> <?php echo $row['name'];  ?></td>
                                            <td id="urunit"> <?php echo $row['unit'];  ?></td>
                                            <td id="urqty"> <?php echo $row['qty'];  ?></td>  
                                        </tr>
                                        <?php }?>
